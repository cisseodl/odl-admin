...
import { Eye, Edit, Trash2, Plus, GraduationCap } from "lucide-react"
...
export function FormationsManager() {
  ...
  const deleteModal = useModal<FormationDisplay>()
  ...
  const handleDeleteFormation = async () => {
    ...
    setError(null)
    try {
      await formationService.deleteFormation(deleteModal.selectedItem.id)
      setFormations((prev) => prev.filter((formation) => formation.id !== deleteModal.selectedItem!.id))
      deleteModal.close()
      toast({
        title: "Succès",
      ...
      const errorMessage = err.message || "Impossible de supprimer la formation."
      setError(errorMessage)
      toast({
        title: "Erreur",
        description: errorMessage,
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <ActionMenu
              onView={() => editModal.open(formation)}
              onEdit={() => editModal.open(formation)}
              onDelete={() => deleteModal.open(formation)}
            />
          )
        },
      },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("routes.formations") || "Formations"}
        description="Gérer les formations de la plateforme"
      />
      ...
      <FormationFormModal
        ...
        description="Créez une nouvelle formation pour organiser vos cours"
      ...
      <ConfirmDialog
        ...
        description={`Êtes-vous sûr de vouloir supprimer la formation "${deleteModal.selectedItem?.title}" ? Cette action est irréversible.`}
